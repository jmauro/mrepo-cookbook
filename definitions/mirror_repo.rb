...
define :mirror_repo,
  ...
  if repo_tags['action'] != 'delete'
    ...
    invalid_options.each do |option|
      next if invalid_array[option]['value'].size != 1
      ...
      raise('>>> [\'mirror_repo\'] ERROR: exiting chef run')
    end
  end
  ...
  if repo_tags['action'] == 'create'
    ...
    template mrepo_config_file do
      ...
      variables(section: repo_name, mrepo: repo_tags)
    ...
    unless repo_tags['iso_url'].nil?
      ...
      iso_url.each do |iso_dvd|
        iso_name = %r{.*\/(.*)$}.match(iso_dvd)[1]
        # --[ Gettin sha256sum file if given by user ]--
        unless repo_tags['iso_sha256sum'].nil? # ~FC023
        ...
        execute "Getting iso: #{iso_name}" do
          ...
          command "(losetup --show -f #{iso_path} >/dev/null 2>&1 && umount #{iso_path} 2>/dev/null >&2 || true) && curl -s -S #{iso_dvd} -o #{iso_path}"
          cwd isodir
          # --[ Chekcing if sha256sum file is present if not test the iso file ]--
          if ::File.exist?("#{iso_path}.sha256sum")
